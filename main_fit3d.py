...
import models.network_fit3d as net
    ...
    with alive_bar(total=bar_length, title='Testing:', length=20, bar='bubbles') as bar:
        with torch.no_grad():
            for bid, batch in enumerate(loader_test):
                p3d_gt      = batch["p3d_gt"].to(device, dtype=torch.float64)   # N, T, 32, 3 T, 32, 2
                trans_in    = batch["trans"].to(device, dtype=torch.float64)    # N, T, 3
                betas_in    = batch["betas"].to(device, dtype=torch.float64)    # N, T, 10
                thetas_in   = batch["thetas"].to(device, dtype=torch.float64)   # N, T, 24, 3 - axis-angle
                ins_params  = batch["intrinsic"] # N, 4
                c, f        = ins_params['c'], ins_params['f']
                ins_params_new = torch.stack((f[0], f[1], c[0], c[1]), dim=1).to(device, dtype=torch.float64)
                ins_mat     = utils.build_intrinsics(ins_params_new).unsqueeze(1)
                N, T, _, _  = p3d_gt.shape
                ...
                cam_R       = torch.stack([torch.stack(row, dim=0) for row in extrinsic['R']], dim=0).permute(2, 0, 1)
                cam_T       = torch.stack([torch.stack(row) for row in extrinsic['T']]).squeeze(0).T
                cam_R       = cam_R[:,None,None,:,:].to(device, dtype=torch.float64)
                cam_T       = cam_T[:,None,None,None,:].to(device, dtype=torch.float64)
                mesh_in_cc  = trans_in.unsqueeze(-2) + mesh_in_ra
                mesh_out_cc = trans_out.unsqueeze(-2) + mesh_out_ra
                mesh_in_wc  = ((mesh_in_cc.unsqueeze(-2) - cam_T) @ cam_R).squeeze(-2)
                mesh_out_wc = ((mesh_out_cc.unsqueeze(-2) - cam_T) @ cam_R).squeeze(-2)
                p3d_in_wc   = ((p3d_in.unsqueeze(-2) - cam_T) @ cam_R).squeeze(-2)
                p3d_17j_wc  = ((p3d_17j.unsqueeze(-2) - cam_T) @ cam_R).squeeze(-2)
                p3d_gt_wc   = ((p3d_gt_17j.unsqueeze(-2) - cam_T) @ cam_R).squeeze(-2)
                if args.input == 'hmr2': ground = -0.02
                if args.input == 'trace': ground = 0.00
                
                p3d_17j_cc  = (p3d_17j.unsqueeze(-2) - cam_T - cam_T @ cam_R.transpose(-2,-1)).squeeze(-2)
                p2d_17j_h   = (ins_mat.repeat(1,T,1,1) @ p3d_17j_cc.transpose(-2,-1)).transpose(-2,-1)
...
if __name__ == "__main__":
    ...
    if args.wandb:
        wandb.init(project = "QuaMo_fit3d_"+args.exp, name = "Model", config = vars(args),
    ...
    trainset, testset = data.create_dataset_fit3d(args, project_path)
    ...
    save_path   = os.path.join(project_path, 'trained_models', 'fit3d', args.exp, 'trace')
    if args.train: main_train(args, init_net, ctrl_net, trainset, smpls["neutral"])
    results_path = os.path.join(project_path, "saved_results", "fit3d_results_"+args.input+".pkl")
