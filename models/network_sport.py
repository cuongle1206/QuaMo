...
class CtrlNet(nn.Module):
    def __init__(self, args, hid_dim: int, J_reg: torch.Tensor, dropout_p: float=0.2):
        ...
        if args.exact: Kp = torch.ones(1,state_dim,1)
        ...
        if self.second:
            Ka = 0.5*torch.ones(1,state_dim,1)
        ...
        self.dt         = 1 # real-time motion capture rate
    ...
    def forward(self, x_t: tuple, x_i: tuple, t: int):
        ...
        # Translation
        m_error     = m_in[:,1] - m
        dv          = kp[:,0] * m_error - kd[:,0] * v + offset[:,0]
        vt          = v + dv * self.dt
        mt          = m + vt * self.dt
